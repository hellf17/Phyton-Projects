...
def extrair_campos_pagina(pdf_path, numero_pagina):
    with open(pdf_path, 'rb') as arquivo_pdf:
        ...
        campos_desejados = {
            ...
            'Bastões': r'BASTÕES|BASTOES \.{3,}: \s+ ([\d.,]+) \s+ %',
            'Segm': r'SEGMENTADOS \.{3,}: \s+ ([\d.,]+) \s+ %',
            'Plaq': r'PLAQUETAS \.{3,}: \s+ ([\d.,]+) \s+ /mm³',
            'K+': r'POTÁSSIO|POTASSIO [\s\S]*?RESULTADO\.{3,}: \s+ ([\d.,]+) \s+ mmol/L',
            'Na+': r'SÓDIO|SODIO [\s\S]*?RESULTADO\.{3,}: \s+ ([\d.,]+) \s+ mmol/L',
            'Ca+': r'CÁLCIO|CALCIO [\s\S]*?RESULTADO\.{3,}: \s+ ([\d.,]+) \s+ mg/dL',
            'Mg+': r'MAGNÉSIO [\s\S]*?RESULTADO\.{3,}: \s+ ([\d.,]+) \s+ mg/dL',
            'Creat': r'CREATININA [\s\S]*?RESULTADO\.{3,}: \s+ ([\d.,]+) \s+ mg/dL',
            'Ur': r'URÉIA|UREIA [\s\S]*?RESULTADO\.{3,}: \s+ ([\d.,]+) \s+ mg/dL',
            'PCR': r'(PCR|PROTEINA[\s\S]*?C[\s\S]*?REATIVA) [\s\S]*?RESULTADO\.{3,}: \s+ ([\d.,]+) \s+ (mg/dL|mg/L)',
        ...
        for campo, padrao in campos_desejados.items():
            correspondencia = re.search(padrao, texto, re.VERBOSE | re.IGNORECASE)
            if correspondencia:
                if correspondencia.group(1).strip() in ['PCR','PROTEINA C REATIVA']:
...
def extrair_id(pdf_path):
    with open(pdf_path, 'rb') as arquivo_pdf:
        ...
        padrao_nome = r'Requisição [\s\S]*?: \s + ([A-Z\s]+) \d'
...
try:
    pdf_path = r'C:\Users\Thoma\Desktop\Phyton\Apps medicina\labsal.pdf'
